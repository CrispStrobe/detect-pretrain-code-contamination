...
from analyze import analyze_data, load_jsonl
from eval import (
    convert_huggingface_data_to_list_dic,
    dump_jsonl,
    process_mmlu,
    process_truthful_qa,
    process_gsm8k,
...
def load_model(name1, name2):
    if "davinci" in name1:
        ...
        model1 = AutoModelForCausalLM.from_pretrained(
            name1, return_dict=True, device_map="auto"
        )
        model1.eval()
    ...
    if "davinci" in name2:
        ...
        model2 = AutoModelForCausalLM.from_pretrained(
            name2, return_dict=True, device_map="auto"
        )
        model2.eval()
...
def calculatePerplexity_gpt3(prompt, modelname):
    ...
    all_prob = [d for d in data.get("token_logprobs") if d is not None]
...
def sample_generation(sentence, model, tokenizer, args):
    ...
    output = model.generate(
        input_ids,
        max_new_tokens=len(sentence.split()) - half_sentence_index,
        min_new_tokens=1,
        num_return_sequences=args["num_z"],
...
def evaluate_RMIA(
    text, target_loss, ref_loss, model1, model2, tokenizer1, tokenizer2, args
):
    # use sample generation
    cur_args = {
        "prefix_length": args.ratio_gen,
        "num_z": 100,
    ...
    # bp()
    result = torch.count_nonzero(target_losses_z < target_loss).item() / len(
        target_losses_z
    )
    return result
...
def inference(
    model1, model2, tokenizer1, tokenizer2, text, ex, modelname1, modelname2, args
):
    pred = {}

    if "davinci" in modelname1:
        p1, all_prob, p1_likelihood = calculatePerplexity_gpt3(text, modelname1)
        p_lower, _, p_lower_likelihood = calculatePerplexity_gpt3(
            text.lower(), modelname1
        )
    else:
    ...
    if "davinci" in modelname2:
        p_ref, all_prob_ref, p_ref_likelihood = calculatePerplexity_gpt3(
            text, modelname2
        )
    else:
        p_ref, all_prob_ref, p_ref_likelihood = calculatePerplexity(
            text, model2, tokenizer2, gpu=model2.device
        )

    # RMIA:
    rmia_result = evaluate_RMIA(
        text,
        p1_likelihood,
        p_ref_likelihood,
        model1,
...
def evaluate_data(
    test_data, model1, model2, tokenizer1, tokenizer2, col_name, modelname1, modelname2
):
...
if __name__ == "__main__":
    ...
    # load model and data
    model1, model2, tokenizer1, tokenizer2 = load_model(
        args.target_model, args.ref_model
    )
